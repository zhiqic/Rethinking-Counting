...
class BasicConv(nn.Module):
    def __init__(self, in_channels, out_channels, use_bn=False, **kwargs):
        ...
        self.use_bn = use_bn
        self.conv = GauConv2d(in_channels, out_channels, bias=not self.use_bn, **kwargs)
...
class BasicDeconv(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_num, stride=1, use_bn=False):
        super(BasicDeconv, self).__init__()
        self.use_bn = use_bn
        self.tconv = nn.ConvTranspose2d(in_channels, out_channels, kernel_num, stride=stride, bias=not self.use_bn)
        self.bn = nn.InstanceNorm2d(out_channels, affine=True) if self.use_bn else None
